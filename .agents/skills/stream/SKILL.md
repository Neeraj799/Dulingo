...
- Peers install on demand per `install_policy` in [`peers.yaml`](peers.yaml): read each peer's `install_policy` before installing. Preserve automatic installation with no prompt only for permitted policies (`silent`); require user confirmation before installing when the policy is `ask`, including for `stream-flutter` (falling back to `fallback_on_decline` if declined). Then route and invoke.
