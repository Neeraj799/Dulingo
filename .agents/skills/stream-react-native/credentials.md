...
2. **Detect whether the selected app already has real data and query feed-group configuration.** For Chat in scope, use a documented non-demo query or paginate `QueryChannels` (e.g., `getstream api QueryChannels --request '{"filter_conditions":{"type":"messaging"}}'`) and for Feeds in scope, paginate `QueryActivities` (e.g., `getstream api QueryActivities`) until finding a record whose identifier does not start with `demo-` or exhausting all results, to prove that no non-demo records exist before allowing seed mutations. In addition, when `UpdateFeedGroup` is in scope, query the existing `foryou` feed-group configuration via `getstream api GetFeedGroup --id foryou`.
