...
2. **Detect whether the selected app already has real data and query feed-group configuration.** For Chat in scope, use a documented non-demo query or paginate `QueryChannels` across all channel types (e.g., `getstream api QueryChannels --request '{"filter_conditions":{}}'` without type restriction, or enumerating and querying all configured channel types) and for Feeds in scope, paginate `QueryActivities` (e.g., `getstream api QueryActivities`) until finding a record whose identifier does not start with `demo-` or exhausting all results, to prove that no non-demo records exist before allowing seed mutations. In addition, when `UpdateFeedGroup` is in scope, query the existing `foryou` feed-group configuration via `getstream api GetFeedGroup --id foryou`.
