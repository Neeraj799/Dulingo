...
2.  Do not include raw `distinct_id` query parameters in deep links. Require a short-lived, server-issued signed handoff token, and include this token in the deep link alongside other properties like UTM parameters.
3.  When the user is redirected to the app, parse the deep link, reject raw `distinct_id` parameters, and validate the token subject before making any identity calls. Using only the validated subject, handle the following cases:

-   The mobile app is already authenticated. In this case, call [`posthog.alias()`](/docs/libraries/js/usage.md#alias) with the validated subject from the handoff token. This associates the two distinct IDs as a single person.
-   The mobile app is unauthenticated. In this case, call [`posthog.identify()`](/docs/libraries/js/usage.md#identifying-users) with the validated subject from the handoff token so pre-login mobile events stay connected to the web session. When the user later logs in on mobile, call `identify()` again with your canonical user ID.
