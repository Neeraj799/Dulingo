...
These properties only tell PostHog how to update person data during ingestion — they aren't kept on the stored event, so you can't filter, break down, or query events by them. To query the values you set, use [person properties](#4-person-profiles-and-properties) instead.

See our [person properties docs](#4-person-profiles-and-properties) for more details on how to work with them and best practices.
...
2.  Do not include raw `distinct_id` query parameters in deep links. Require a short-lived, server-issued signed handoff token that binds each token to its intended authenticated user or audience, and include this token in the deep link alongside other properties like UTM parameters.
3.  When the user is redirected to the app, parse the deep link, reject raw `distinct_id` parameters, and compare the token's binding with the currently authenticated user before making any identity call. Reject subject mismatches before invoking `posthog.alias()` or `posthog.identify()`, while preserving the existing validated-subject behavior for matching users and unauthenticated flows:

-   The mobile app is already authenticated. Compare the token binding with the currently authenticated user; reject subject mismatches before invoking `posthog.alias()`. If matched, call [`posthog.alias()`](/docs/libraries/js/usage.md#alias) with the validated subject from the handoff token. This associates the two distinct IDs as a single person.
-   The mobile app is unauthenticated. Ensure the token is valid for an unauthenticated flow, rejecting mismatches before invoking `posthog.identify()`. In this case, call [`posthog.identify()`](/docs/libraries/js/usage.md#identifying-users) with the validated subject from the handoff token so pre-login mobile events stay connected to the web session. When the user later logs in on mobile, call `identify()` again with your canonical user ID.
...
class DeepLinkIdentityManager {
    ...
    func handleDeepLink(_ url: URL, isAuthenticatedOnMobile: Bool) {
        // Reject raw distinct_id query parameters. Use short-lived signed handoff token instead.
        guard let token = URLComponents(url: url, resolvingAgainstBaseURL: true)?
            .queryItems?.first(where: { $0.name == "handoff_token" })?.value,
            let validatedSubject = validateHandoffToken(token) else {
            return
        }
        if isAuthenticatedOnMobile {
            // The mobile app already knows the current user.
            // Alias the validated subject to that user.
            PostHogSDK.shared.alias(validatedSubject)
        } else {
            // Reuse the validated subject until login on mobile.
            PostHogSDK.shared.identify(validatedSubject)
        }
    }
    // MARK: - Login/Signup
...
object DeepLinkIdentityManager {
    ...
    fun handleDeepLink(uri: Uri, isAuthenticatedOnMobile: Boolean) {
        // Reject raw distinct_id query parameters. Use short-lived signed handoff token instead.
        val token = uri.getQueryParameter("handoff_token") ?: return
        val validatedSubject = validateHandoffToken(token) ?: return
        if (isAuthenticatedOnMobile) {
            // The mobile app already knows the current user.
            // Alias the validated subject to that user.
            PostHog.alias(validatedSubject)
        } else {
            // Reuse the validated subject until login on mobile.
            PostHog.identify(validatedSubject)
        }
    }
    // Login/Signup
